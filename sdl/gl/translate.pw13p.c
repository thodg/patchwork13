...
*
  ...
  translate.pw13p.c  -  SDL  opengl translate patch
...
static void pw13_sdl_translate_pump (Pw13_Patch *p, Pw13_Time *time)
{
  pw13_pump_sources (p, time);
  glTranslatef(pw13_float_input_val(p->input+1),
...
int pw13_dl_patch_class_init (const char *name, Pw13_Patch *p, void *param)
{
  ...
  pw13_float_input ("x", p->input+1, p, 0);
  pw13_float_input ("y", p->input+2, p, 0);
  pw13_float_input ("z", p->input+3, p, -6.0f);
  pw13_void_output ("()", p->output, p);
  p->pump = pw13_sdl_translate_pump;
  return 1;
}
