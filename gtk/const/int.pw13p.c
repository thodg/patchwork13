...
*
  ...
  int.pw13p.c  -  int constant patch
...
static void build_open (Pw13_Method *m, void *call_param)
{
  ...
  spinner = gtk_spin_button_new_with_range (INT_MIN, INT_MAX, 1);
  gtk_spin_button_set_value (GTK_SPIN_BUTTON (spinner), -1000);
...
static void build_closed (Pw13_Method *m, void *call_param)
{
  ...
  w->adjustment = GTK_ADJUSTMENT (gtk_adjustment_new (10, -1000, 1000,
						      1, 10, 10));

  w->unused = gtk_hbox_new (FALSE, 0);

  w->closed.spinner = gtk_spin_button_new (w->adjustment, 1, 0);
  ...
  gtk_widget_set_size_request (GTK_WIDGET (w->closed.hscale), 72, 8);
...
static void pw13_gtk_const_int_import (Pw13_Method *m, void *call_param)
{
  ...
  if (node) {
    ...
    pw13_int_output_val (p->output[0]) = value;
  }
}


static void pw13_gtk_const_int_export (Pw13_Method *m, void *call_param)
{
  Pw13_Export * x = (Pw13_Export*) call_param;
  Pw13_Patch *p = (Pw13_Patch*) m->name->object;
  p_patch_widgets w = (p_patch_widgets) p->param;
  int min = (int) w->adjustment->lower;
  int max = (int) w->adjustment->upper;
  int val = (int) w->adjustment->value;
  xmlTextWriterStartElement (x->xml, BAD_CAST "pw13_gtk_const_float");
  xmlTextWriterWriteFormatAttribute (x->xml, BAD_CAST "min", "%i", min);
  xmlTextWriterWriteFormatAttribute (x->xml, BAD_CAST "max", "%i", max);
  xmlTextWriterWriteFormatAttribute (x->xml, BAD_CAST "value", "%i", val);
...
int pw13_dl_patch_class_init (const char *name, Pw13_Patch *p, void *param)
{
  ...
  pw13_patch_method_add (p, "xml export", pw13_gtk_const_int_export, NULL);
  pw13_patch_method_add (p, "xml import", pw13_gtk_const_int_import, NULL);
