...
*
  ...
  mul.pw13p.c  -  Multiplication on floats
...
static void patch_pump (Pw13_Patch *p, Pw13_Time *time)
{
  ...
  pw13_float_output_val (p->output[0]) =
    ...
    * pw13_float_input_val (p->input+1); 
}
...
int pw13_dl_patch_class_init (const char *name, Pw13_Patch *p, void *param)
{
  ...
  pw13_float_input ("A", p->input, p, 0.0f);
  pw13_float_input ("B", p->input+1, p, 0.0f);
  pw13_float_output ("A*B", p->output, p);
  p->pump = patch_pump;
  param = param;
  return 1;
}
